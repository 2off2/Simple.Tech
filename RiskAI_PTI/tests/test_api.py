...
# Importa diretamente do módulo FastAPI
from fastapi import FastAPI

# Cria uma instância de teste da aplicação
test_app = FastAPI(title="RiskAI API Test", description="API para testes")

# Importa os routers
from api.endpoints import data
# Importe outros routers conforme necessário

# Registra os routers na aplicação de teste
test_app.include_router(data.router, prefix="/api/data", tags=["data"])
# Registre outros routers conforme necessário

# Mock do state para testes
class MockState:
    UPLOAD_DIR = os.path.join(os.path.dirname(__file__), "test_uploads")
    global_processed_df = None
    global_prediction_model = None
    global_historical_stats = None
...
# Cliente de teste usando a aplicação de teste
